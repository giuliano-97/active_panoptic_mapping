...
class Vertex {
 public:
  ...
  Vertex(float3 v, int _id, byte3 color);
  ~Vertex();
...
Vertex::Vertex(float3 v, int _id, byte3 color) {
  position = v;
  id = _id;
  this->color = color;
...
void AddVertex(std::vector<float3>& vert, std::vector<byte3>& colors) {
  for (unsigned int i = 0; i < vert.size(); i++) {
    Vertex* v = new Vertex(vert[i], i, colors[i]);
  }
}
...
void ProgressiveMesh(std::vector<float3>& vert,
                     std::vector<byte3>& vert_colors,
                     std::vector<tridata>& tri,
                     std::vector<int>& objid,
                     std::vector<int>& map,
                     std::vector<int>& permutation) {
  AddVertex(vert, vert_colors);  // put input data into our data structures
  AddFaces(tri, objid);
...
void PermuteVertices(std::vector<float3>& vert,
  ...
  for (i = 0; i < vert.size(); i++) {
    ...
    colors[permutation[i]] = temp_colors[i];
  }
#ifdef DEBUG
...
int main(int argc, char* argv[]) {
  ...
  LoadDataFromFile(argv[1], vert, vert_colors, tri, objid);
  ...
  ProgressiveMesh(vert, vert_colors, tri, objid, collapse_map, permutation);
  PermuteVertices(vert, vert_colors, tri, permutation);
  WriteDataToFile(argv[2], vert, vert_colors, tri, objid, collapse_map, atoi(argv[3]));
