...
class MaXDeepLabPredictor(PredictorBase):
    ...
    def _get_detected_masks(
        ...
        thresholded_mask = tf.cast(
            tf.greater_equal(
                ...
                MaXDeepLabPredictor.TRANSFOMER_CLASS_CONFIDENCE_THRESH,
            ),
            tf.float32,
        )
    ...
    def __call__(self, image: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        class_logits = tf.squeeze(
            res_dict[MaXDeepLabPredictor.TRANSFOMER_CLASS_LOGITS_KEY], axis=0
        )

        # Filter masks by class score
        detected_mask_indices, _, _ = MaXDeepLabPredictor._get_detected_masks(
            class_logits
        )
        ...
        panoptic_pred = tf.squeeze(
            res_dict[MaXDeepLabPredictor.PANOPTIC_PRED_KEY], axis=0
        )
        ...
        semantic_logits = tf.squeeze(
            res_dict[MaXDeepLabPredictor.SEMANTIC_LOGITS_KEY],
            axis=0,
        )
